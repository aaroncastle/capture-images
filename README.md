...
2. `config.json`对象由以下部分组成：
   ...
   - timeout: 图片地址的下载超时时间，单位毫秒。默认500。
...
3. 域名对象的组成：
   ...
   | timeout   | Number          | 500  | 否       | 有些页面打开速度较慢，可以自由设置超时时间。单位是毫秒。超过超时时间程序不会停止，但会将超时错误写入`src/logs/error.log`,**超时将会使当前页面下的一系列分类与分页下的图片无法获取**。根据自己当时的网络速度自行设置，一般建议1500以上。优先级大于`config.json`中的`timeout` |
